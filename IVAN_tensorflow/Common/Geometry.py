...
def rapid_rejection_test(A1, A2, B1, B2):
    return (min(A1[0], A2[0]) <= max(B1[0], B2[0])) and (min(B1[0], B2[0]) <= max(A1[0], A2[0])) and (min(A1[1], A2[1]) <= max(B1[1], B2[1])) and (min(B1[1], B2[1]) <= max(A1[1], A2[1]))
...
def is_segment_cross(A1, A2, B1, B2):
    ...
    if cross_product(AB, AC) * cross_product(AB, AD) < 0 or cross_product(CD, CA) * cross_product(CD, CB):
        return True
    return False
...
def draw_lines(frame, lines, flag):
    for j in range(len(lines)):
        if flag[j] >= 1:
            cv2.line(frame, (lines[j][0], lines[j][1]), (lines[j][2], lines[j][3]), (255, 0, 0), 5)
