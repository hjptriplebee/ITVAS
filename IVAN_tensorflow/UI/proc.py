...
def pipeline(frame, frame_num, lines, cnt, results, debug, svm, model):
    ...
    for trk in tracker_list:
        if (trk.hits >= min_hits) and (trk.no_losses <= max_age):
            if debug:
                ...
                is_get_class = True
                cur_box=trk.cur_box
                for i,p in enumerate(cur_box):
                    #print(frame.shape[0],frame.shape[1])
                    if p<0 or p>frame.shape[(i+1)%2]:
                        is_get_class = False
                        break
                if is_get_class==True:
                    top, bottom = min(cur_box[1], cur_box[3]), max(cur_box[1], cur_box[3])
                    left, right = min(cur_box[0], cur_box[2]), max(cur_box[0], cur_box[2])
            ...
            for j in range(len(lines)):
                trk, cnt, results, flag_temp = Count.counting(trk, lines[j][0:2], lines[j][2:4], cnt, j, results,svm,original_frame)
                flag[j] += flag_temp
...
def proc(label, video, lines, svm, tf_model, gap = 1, debug = False):
    ...
    while True:
        ...
        if ret:
            frame, cnt, results = pipeline(frame, frame_num, lines, cnt, results, debug, svm, tf_model)
            print("--"*40)
